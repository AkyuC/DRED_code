...
def plot_survival_time(config):
    ...
    plt.plot(np.arange(len(lifetime)), np.ones(len(lifetime))*2259.1, label='Greedy')
    plt.plot(np.arange(len(lifetime)), np.ones(len(lifetime))*2141, label='Rotate')
    plt.plot(np.arange(len(lifetime)), np.ones(len(lifetime))*1514.6, label='Random')
    plt.legend()
    plt.ylabel("lifetime")
    plt.xlabel("episode")
    plt.title('lifetime')
