...
def plot_static():
    ...
    #         1059, 2582, 2428,]
    data_min = [1418, 1127, 1658, 987, 1185,
            1059, 2582, 2449,]
    ...
    plt.text(x[0]+0.15, data_mean[0], '%d' % data_mean[0], color='black', fontdict={'fontsize':14})
    plt.text(x[1]+0.1, data_mean[1]-100, '%d' % data_mean[1], color='black', fontdict={'fontsize':14})
    plt.text(x[2]+0.15, data_mean[2], '%d' % data_mean[2], color='black', fontdict={'fontsize':14})
    plt.text(x[3]+0.15, data_mean[3]+100, '%d' % data_mean[3], color='black', fontdict={'fontsize':14})
    ...
    plt.ylabel("Number of Lifetime / Rounds", fontdict={'family':'serif','size':14})
    plt.xlabel("Scheme", fontdict={'family':'serif','size':14})
...
def plot_total_energy():
    ...
    en_total_ours = np.loadtxt("Ours_total_energy")
    en_total_n13 = np.loadtxt("Static_n13_total_energy")
    plt.rcParams['figure.figsize'] = (7,5)
    plt.rcParams['font.size'] = 16
    plt.plot(np.arange(len(en_total_ours)), en_total_ours, color="#82cafc", label="Ours", alpha = 0.75, linewidth = 2)
    plt.plot(np.arange(len(en_total_random)), en_total_random, color='orange', label="Random", alpha = 0.75, linewidth = 2)
    plt.plot(np.arange(len(en_total_greedy)), en_total_greedy, color='green', label="Greedy", alpha = 0.75, linewidth = 2)
    plt.plot(np.arange(len(en_total_n13)), en_total_n13, color='red', label="n13", alpha = 0.75, linewidth = 2)
    plt.xticks(np.arange(len(en_total_ours)), np.arange(len(en_total_ours))*10)
    plt.ylabel("Total Network Energy / J", fontdict={'family':'serif'})
    plt.xlabel("Network Lifetime / Rounds", fontdict={'family':'serif'})
